...
def compare(manifest, staged, deployed, session=None, evidence=None, working=None, definition=None, working_version=None):
    """session is current process identity, never inferred from installed files.

    Evidence must be collected from the live diagnostic bridge or a log proven
    to belong to that process; adapters supply PID and process start identity.
    Equal runtime version alone is explicitly weaker than verified payload bytes.
    """
    changes = []
    stage_errors = []
    working_changes = []
    ...
    loaded = {'status': 'not-running', 'version': None, 'contents_verified': False}
    if session:
        loaded['status'] = 'unverified'
        if evidence and evidence.get('session') == session and evidence.get('module') == manifest['module']:
            loaded['version'] = evidence.get('version')
            if loaded['version']:
                loaded['status'] = 'version-match' if loaded['version'] == manifest['version'] else 'version-mismatch'
            # Hashes must represent bytes captured at load, not a subsequent disk read.
            hashes = evidence.get('loaded_file_hashes')
            loaded['contents_verified'] = loaded['status'] == 'version-match' and bool(hashes) and hashes == manifest['files']
    ...
    needs_attention = bool(stage_errors or working_changes or stale_version or changes or (session and (loaded['status'] != 'version-match' or not loaded['contents_verified'])))
    return {
        ...
        'loaded': loaded, 'needs_attention': needs_attention,
        'options': ([
...
def main():
    ...
    parser.add_argument('--runtime-evidence', type=Path, help='Evidence from live bridge/current-session log')
    ...
    if args.session is None and os.name == 'nt':
        ...
        if raw:
            ...
            if isinstance(sessions, list):
                ...
                sessions = sessions[0]
            session = sessions
    result = compare(read(args.manifest), args.staged, args.deployed, session, read(args.runtime_evidence), args.working, read(args.definition), args.working_version)
