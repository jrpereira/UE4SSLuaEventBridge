...
class PreflightTests(unittest.TestCase):
    def test_same_version_different_bytes_and_old_session(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            evidence = {'module': 'Example', 'version': '1.2.3', 'session': {'pid': 7, 'started_at': '2026-09-17T10:00:00Z'}}
            result = preflight.compare(manifest, staged, deployed, session, evidence)
            self.assertEqual(result['deployed_differences'], ['main.lua'])
            self.assertEqual(result['loaded']['status'], 'unverified')
            evidence['session'] = session
            result = preflight.compare(manifest, staged, deployed, session, evidence)
            self.assertEqual(result['loaded']['status'], 'version-match')
            self.assertFalse(result['loaded']['contents_verified'])
            evidence['version'] = '0.0.1'
            evidence['loaded_file_hashes'] = manifest['files']
            result = preflight.compare(manifest, staged, deployed, session, evidence)
            self.assertEqual(result['loaded']['status'], 'version-mismatch')
            self.assertFalse(result['loaded']['contents_verified'])
