...
try {
    ...
    if (-not $SkipTests) {
        ...
        Invoke-Checked cmake @('--build', 'build/tests', '--target', 'SessionAliasIndexTests', 'LifecycleRegistryTests', 'BindingSnapshotTests', 'WeakObjectPtrTests', 'QueueBuffersTests', 'QueueDispatchScheduleTests', 'DispatchBudgetTests', 'NativeBackendLifecycleTests', 'LegacyInstallMigrationTests')
